...
![**Figure 1: The NWC-DP homepage.**](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure1.jpg)
...
![**Figure 2: Selecting the Huc layer and gage. layer.**](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure2.jpg)
...
![**Figure 3: Selecting the Huc layer and gage. layer.**](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure3.jpg)
...
![**Figure 4: NWIS gage location and time series. graph.**](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure4.JPG)
...
<code id="graphcode1" style="white-space: normal;text-align:left;">
library(NWCEd)
<br>library(leaflet)
<br>huc12<-"160202030505"
<br>NWCwatershed<-getNWCWatershed(huc="160202030505",local=FALSE)
<br>coords<-NWCwatershed$features[[1]]$geometry$coordinates[[1]][[1]]  
<!-- Break -->
coords<-as.data.frame(matrix(unlist(coords),nrow=length(coords),byrow = T))
<!-- Break -->
names(coords)<-c('lon','lat')
...
<div style="text-align:justify; clear: left;">
<code id="graphcode2" style="white-space: normal;text-align:left;">
<br>library(ggplot2)
<br>library(NWCEd)
<br>huc12<-"160202030505"
<br>NWCdata<-getNWCData(huc=huc12)
<br>annual_dataet<-annualize(NWCdata$et, method=sum)
<br>annual_dataet$group<-'ET'
<br>annual_dataprcp<-annualize(NWCdata$prcp, method=sum)
<br>annual_dataprcp$group<-'P'
<br>annual_union<-merge(annual_dataet,annual_dataprcp,by.x="year",by.y="year")
<br>annual_diff<-as.data.frame(annual_union$data.y - annual_union$data.x)
<br>names(annual_diff) = c('data')
<br>annual_diff$group<-'diff'
<br>annual_diff$year<-annual_union$year
<br>library(scales)
<br>library(ggplot2)
<br>require(stats)
<br>plotData<-rbind(annual_dataet,annual_dataprcp,annual_diff)
<br>ggplot(plotData, aes(x=year, y=data, group=group,colour = group)) +  geom_line() +
  scale_x_discrete(name = "year") + ylab("mm") + 
  theme(axis.text.x = element_text(angle = 45)) +
  labs(title = "Difference Between Precipitation and ET", colour = "Explanation") +
  theme(legend.position = c(0.91, 0.83)) + scale_color_manual(values=c("#990066", "#339966", "#FF0000"))
...
![**Figure 5: Utah State Park during the 1983 flood (http://www.utahlake.gov/1983-flooding/).**](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure5.jpg)
</div>
<div style="text-align:justify">
...
![**Figure 6: Utah Lake surface elevations (http://www.utahlake.gov/1983-flooding/).**](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure6.jpg)
...
<div style="text-align:justify; clear: left;">
<code id="graphcode3" style="white-space: normal;">
<br>library(NWCEd)
<br>library(ggplot2)
<br>NWCdata<-getNWCData(huc="031501070601", local = FALSE)
<br>NWCwatershed<-getNWCWatershed(huc="031501070601",local=FALSE)
<br>areasqft<-NWCwatershed$features[[1]]$properties$areaacres*43560
<br>NWCdata$et$data<-NWCdata$et$data*0.0032808
<br>annual_dataet<-annualize(NWCdata$et, method=sum)
<br>annual_dataet$group<-'ET'
<br>NWCdata$prcp$data<-NWCdata$prcp$data*0.0032808
<br>annual_dataprcp<-annualize(NWCdata$prcp, method=sum)
<br>annual_dataprcp$group<-'P'
<br>NWCdata$streamflow$data<-NWCdata$streamflow$data*60*60*24/areasqft
<br>annual_datastrf<-annualize(NWCdata$streamflow,method=sum)
<br>annual_datastrf$group<-'Q'
<br>annual_union<-merge(annual_dataet,annual_dataprcp,by.x="year",by.y="year")
<br>annual_union<-merge(annual_union,annual_datastrf,by.x="year",by.y="year")
<br>names(annual_union)<-c("year","ET","groupET","P","groupP","Q","groupQ")
<br>annual_diff<-as.data.frame(annual_union$P - annual_union$ET - annual_union$Q)
<br>names(annual_diff) = c('data')
<br>annual_diff$group<-'diff'
<br>annual_diff$year<-annual_union$year
<br>plotData<-rbind(annual_dataet,annual_dataprcp,annual_datastrf,annual_diff)
<br>ggplot(plotData, aes(x=year, y=data, group=group,colour = group)) +  geom_line() +
  scale_x_discrete(name = "year") + ylab("ft") +
  theme(axis.text.x = element_text(angle = 45)) +
  labs(title = "Basic Diff Plot", colour = "Explanation") +
...
<div style="text-align:justify; clear: left;">
<code id="graphcode4" style="white-space: normal;">
<br>library(dplyr)
<br>NWCdata<-getNWCData(huc="031501070601", local = FALSE)
<br>NWCwatershed<-getNWCWatershed(huc="031501070601",local=FALSE)
<br>areasqft<-NWCwatershed$features[[1]]$properties$areaacres*43560
...
<br>annual_diff$group<-'diff'
<br>annual_diff$year<-annual_union$year
<br>annual_diff = annual_diff%>%
  <br>mutate(pos = data >= 0)
<br>ggplot(annual_diff, aes(x=year, y=data, fill=pos)) + geom_bar(stat='identity', position = 'identity', color='black', size=0.25)+ scale_fill_manual(values=c('#FFDDDD','#CCEEFF'), guide = FALSE) + xlab("Year") + ylab("Annual Storage (ft)") + ggtitle("Annual Storage from 2000 to 2010") 
</code>
</div>
<br>
...
<div style="text-align:justify; clear: left;">
<code id="graphcode5" style="white-space: normal;">
<br>huc12<-"031601090601"
<br>library(NWCEd)
<br>library(leaflet)
<br>NWCwatershed<-getNWCWatershed(huc=huc12,local=FALSE)
<br>coords<-NWCwatershed$features[[1]]$geometry$coordinates[[1]][[1]]
<br>coords<-as.data.frame(matrix(unlist(coords),nrow=length(coords),byrow = T))
<br>names(coords)<-c('lon','lat')
<br>leaflet() %>% 
  setView(lng = mean(coords$lon), lat = mean(coords$lat), zoom = 9) %>%
  addTiles("http://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}") %>%
  addWMSTiles(baseUrl = "http://cida.usgs.gov/nwc/geoserver/gwc/service/wms", 
...
csv_data<-download.file("https://rawgit.com/jnelson7/NWCEd/43da87970865520fa698640f726e8762b348e95c/inst/extdata/doublemassplot2sheet1.csv", quiet = TRUE, destfile = './doublemassplot2sheet1.csv')
NWCdataprcpsumannual4<-read.csv('./doublemassplot2sheet1.csv')

csv_data2<-download.file("https://rawgit.com/jnelson7/NWCEd/43da87970865520fa698640f726e8762b348e95c/inst/extdata/doublemassplot2sheet2.csv", quiet = TRUE, destfile = './doublemassplot2sheet2.csv')
...
<div style="text-align:justify; clear: left;">
<code id="graphcode6" style="white-space: normal;">
NWCdata<-getNWCData(huc="150601060306", local = FALSE)
NWCdataprcp1<-NWCdata$prcp
NWCdataprcp1$data<-NWCdataprcp1$data*.0393701
NWCdataprcpsumannual1<-annualize(NWCdataprcp1, method = sum)
...
NWCdataprcpsumannual4<-read.csv('./doublemassplot2sheet2.csv')
...
<div style="text-align:justify; clear: left;">
<code id="graphcode7" style="white-space: normal;">
NWCdata<-getNWCData(huc="150601060306", local = FALSE)
NWCdataprcp1<-NWCdata$prcp
NWCdataprcp1$data<-NWCdataprcp1$data*.0393701
NWCdataprcpsumannual1<-annualize(NWCdataprcp1, method = sum)
...
correcteddata<- read.csv('./doublemassplot2sheet2.csv')

uncorrecteddata<- read.csv('./doublemassplot2sheet1.csv')

file.remove('./doublemassplot2sheet1.csv')
file.remove('./doublemassplot2sheet2.csv')

correcteddata$group<-'Corrected'
drops<-c("NA.")
correcteddata<- correcteddata[,!(names(correcteddata) %in% drops)]
correcteddata<-subset(correcteddata, select = -c(X))

uncorrecteddata$group<-'Uncorrected'
drops<-c("NA.")
uncorrecteddata<- uncorrecteddata[,!(names(uncorrecteddata) %in% drops)]
uncorrecteddata<-subset(uncorrecteddata, select = -c(X))
...
bounddata<-rbind(datadiff,correcteddata,uncorrecteddata)

ggplot(bounddata, aes(x=year, y=data, group=group,colour = group)) +  geom_line() +
...
<div style="text-align:justify; clear: left;">
<code id="graphcode8" style="white-space: normal;">
library(scales)
library(ggplot2)
correcteddata<- read.csv('./doublemassplot2sheet2.csv')
uncorrecteddata<- read.csv('./doublemassplot2sheet1.csv')
...
<div style="text-align:justify; clear: left">
* Find the 12-Digit HUC ID for HUC that encompasses Gunnison National Forest in Colorado.
<br>
<a id="codetoggle1" onclick="togglecode('hint1','codetoggle1');">Hint</a>
</div>
<div style="text-align:justify; clear: left">
<code id="hint1">
Search for "Callow Creek CO" and select "Callow Creek CO Mesa County"
![](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure7.JPG)
</code>
</div>
* Verify the selected 12-Digit HUC contains streamflow data.
<br>
<a id="codetoggle1" onclick="togglecode('hint2','codetoggle1');">Hint</a>
</div>
<div style="text-align:justify; clear: left">
<code id="hint2">
![](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure8.JPG)
</code>
</div>
* Plot an annual time series graph for the selected HUC which shows ET, precipitation, and runoff datasets in R using the code block below.  _Hint: make sure and change the HUC ID in the second and third lines of code.  Update the title of the plot at the bottom of the code block._  Look at the ET line.  How do you explain the two major low points?  Research the site including weather events dating back to 1980.  Can you find any information that could help support the graph you have produced?
```{r Problem 2 plot, echo=TRUE, eval=FALSE }
library(ggplot2)
# Download datasets for selected HUC ID
NWCdata<-getNWCData(huc="150601060306", local = FALSE)
# Download HUC geometry information for selected HUC ID
NWCwatershed<-getNWCWatershed(huc="150601060306",local=FALSE)
# Convert HUC area to square feet
areasqft<-NWCwatershed$features[[1]]$properties$areaacres*43560
# Separate ET data and convert to feet
NWCdata$et$data<-NWCdata$et$data*0.0032808
# Annualize ET dataset and assign 'ET' group name
annual_dataet<-annualize(NWCdata$et, method=sum)
annual_dataet$group<-'ET'
# Separate precipitation data and convert to feet
NWCdata$prcp$data<-NWCdata$prcp$data*0.0032808
# Annualize precipitation dataset and assign 'P' group name
annual_dataprcp<-annualize(NWCdata$prcp, method=sum)
annual_dataprcp$group<-'P'
# Separate streamflow dataset, normalize, and convert to feet
NWCdata$streamflow$data<-NWCdata$streamflow$data*60*60*24/areasqft
# Removes unnecessary columns
datastrf<-subset(NWCdata$streamflow, select = -c(agency_cd,site_no,data_00060_00003,cd_00060_00003))
# Updates the "date" column name
colnames(datastrf)[1]<-"date"
# Annualizes the streamflow dataset and assigns it 'Q' group name
annual_datastrf<-annualize(datastrf,method=sum)
annual_datastrf$group<-'Q'
# Extracts subset of streamflow data to reduce size
annual_datastrf<-annual_datastrf[c(75:111),]
# Combines datasets into 1 data frame
annual_union<-merge(annual_dataet,annual_dataprcp,by.x="year",by.y="year")
annual_union<-merge(annual_union,annual_datastrf,by.x="year",by.y="year")
# Assigns column headers
names(annual_union)<-c("year","ET","groupET","P","groupP","Q","groupQ")
# Create diff dataset by subtracting ET and streamflow from precipitation dataset
annual_diff<-as.data.frame(annual_union$P - annual_union$ET - annual_union$Q)
# Assign column name to diff dataset
names(annual_diff) = c('data')
# Assign 'diff' gropu name to diff dataset
annual_diff$group<-'diff'
# Assigns the same range of years to diff dataset as is being used by other datasets
annual_diff$year<-annual_union$year
# combines ET, precipitation, and streamflow datasets into 1 dataset
plotData<-rbind(annual_dataet,annual_dataprcp,annual_datastrf,annual_diff)
# Plots annual time series
ggplot(plotData, aes(x=year, y=data, group=group,colour = group)) +  geom_line() +
  scale_x_discrete(name = "year") + ylab("ft") +
  theme(axis.text.x = element_text(angle = 45)) +
  labs(title = "Annual Time Series Plot for HUC # 140200050803", colour = "Explanation") +
  scale_color_manual(values=c("#990066", "#339966", "#FF0000", "#3300FF"))
```
<div style="text-align:center">
<a id="codetoggle9" onclick="togglecode('answer1','codetoggle9');">Answer</a>
</div>
<div style="text-align:justify; clear: left">
<code id="answer1">
![](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure9.jpeg)
</code>
</div>
<br>
...
Using the same HUC from **Problem 2** plot a bar graph of the storage (diff) by copying and pasting the code block into your R console as shown below.  _Hint: Change the HUC ID in the third and fourth lines of code.  Be sure to change the plot title to accurately reflect the dateset dates_.  Assuming 100% of the storage is available to distribute to a population living within the HUC boundary, write a paragraph to the water manager with a recommendation for the population size that the storage could support from 2000 to 2014.  Include the plot you created.
```{r Problem 3, echo=TRUE, eval=FALSE}
library(dplyr)
library(ggplot2)
# Download datasets for selected HUC ID
NWCdata<-getNWCData(huc="150601060306", local = FALSE)
...
# Assigns the same range of years to diff dataset as is being used by other datasets
annual_diff$year<-annual_union$year
# Adds a column of True and False values associated with respective positive and negative values in the original dataset
annual_diff = annual_diff%>%
  mutate(pos = data >= 0)
# Plots the data
ggplot(annual_diff, aes(x=year, y=data, fill=pos)) + geom_bar(stat='identity', position = 'identity', color='black', size=0.25)+ scale_fill_manual(values=c('#FFDDDD','#CCEEFF'), guide = FALSE) + xlab("Year") + ylab("Annual Storage (ft)") + ggtitle("Annual Storage from 2000 to 2014") 
```
<div style="text-align:center">
...
<div style="text-align:justify; clear: left">
<code id="answer2">
![](https://cdn.rawgit.com/dblodgett-usgs/NWCEd/master/inst/Lab4_NWC-DP_Images/Figure10.jpeg)
</code>
</div>
