...
How do we know if our precipitation data is good to use?  In the field, there is possibility that stream gages are moved or are damaged over a period of time.  Problems that occur in the field must be taken into consideration and the data 




```{r double mass plot, echo=FALSE}

NWCdata<-getNWCData(huc="031501070601", local = FALSE)
NWCdataprcp<-NWCdata$prcp
NWCdataprcp1<-NWCdata$prcp
# Converting daily precipitation to annual preciptitation for Site 1
NWCdataprcpsumannual1<-annualize(NWCdataprcp1, method = sum)
# Cumulatizing the annual precipitation for Site 1
...
plot(NWcdataprcpsumannualcum1, annualaveragecum)
plot(NWcdataprcpsumannualcum2, annualaveragecum)
plot(NWcdataprcpsumannualcum3, annualaveragecum)
plot(NWcdataprcpsumannualcum4, annualaveragecum)
```
